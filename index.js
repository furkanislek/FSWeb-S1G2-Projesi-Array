...
function dizi25Cesitmi(/*kod buraya*/) {
  const lengthControl = orijinalTatlar.length;
  const control = lengthControl == 25 ? true : false;
  return control;
}
...
function sonCesitiKaldir(/*kod buraya*/) {
  /*kod buraya*/
  var arr3 = orijinalTatlar.slice(0, -1);
  return arr3;
}
...
function ortalamaKelimeSayisi(tatlar) {
  /*kod buraya*/
  var ortArr = kopyala(tatlar);
  var iki = 0;
  var bir = 0;

  for(var i = 0; i< ortArr.length; i++){
    if(ortArr[i].includes(" ")){
     iki++
    }else{
      bir++
    }
  }
  var res = (bir + iki*2)/(bir+iki)
  return res;
}
